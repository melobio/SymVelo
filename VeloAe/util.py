...
from .model import leastsq_pt, leastsq_generalized
...
def fit_model(args, adata, model, inputs, xyids=None, device=None, edge_index=None):
...
def construct_nb_graph_for_tgt(src_adata, tgt_adata, g_basis="SU", g_rep_dim=100):
    ...
    if type(src_adata.X) == type(src_adata.layers['Ms']):
        tgt_adata.obsm['X_pca'] = PCA(n_components=g_rep_dim).fit_transform(np.hstack([basis_dict[k] for k in g_basis]))
    else:
        tgt_adata.obsm['X_pca'] = PCA(n_components=g_rep_dim).fit_transform(np.hstack([basis_dict[k].toarray() for k in g_basis]))
...
def new_adata(adata, x, s, u, v=None,
    ...
    new_adata = construct_nb_graph_for_tgt(adata, new_adata, g_basis, g_rep_dim)
...
def train_step_AE(Xs, model, optimizer, xyids=None, device=None, aux_weight=1.0, rt_all_loss=False, perc=[5, 95],
                  norm_lr=False, edge_index=None):
    ...
    if len(xyids) == 2:
        ...
        loss += vloss
    if len(xyids) == 3:
        s, u, c = model.encoder(Xs[xyids[0]]), model.encoder(Xs[xyids[1]]), model.encoder(Xs[xyids[2]])
        _, _, vloss = leastsq_pt(
            u, c,
            fit_offset=True,
            perc=perc,
...
def get_veloAE(
    ...
    edge_weight = torch.DoubleTensor(conn[xs, ys]).view(-1).to(device)
    ...
    return model.to(device), edge_index
